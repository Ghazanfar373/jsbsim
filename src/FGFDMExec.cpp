...
#include "models/TacticalEnvironment.h"
#include "DDSInterface/DDSInterface.h"
#include "models/ElevationModel.h"
...
//  instance->Tie("simulation/do_trim_analysis", this, (iPMF)0, &FGFDMExec::DoTrimAnalysis, false);
  ...
  instance->Tie("simulation/deltaTime", &dT);
...
bool FGFDMExec::Allocate(void)
{
  ...
  Models[eTacticalEnvironment] = new TacticalEnvironment(this);
  Models[eElectricalSystem] = new ElectricalSystem(this);
  Models[eElevationModel]   = new ElevationModel(this);
  Models[eDDSInterfaces]     = new DDSInterface(this);
  ...
  Tactical = (TacticalEnvironment *)Models[eTacticalEnvironment];
  ElectricalSystemInstance = (ElectricalSystem*)Models[eElectricalSystem];
  ElevationModelInstance = (ElevationModel*)Models[eElevationModel];
  dds = (DDSInterface*)Models[eDDSInterfaces];
...
void FGFDMExec::LoadInputs(unsigned int idx)
{
  ...
  case eAccelerations:
    ...
    break;
  case eTacticalEnvironment:
      break;
  case eElevationModel:
      break;
  case eDDSInterfaces:
      break;
  default:
