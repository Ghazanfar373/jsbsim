...
void FGWinds::SetWindspeedKts(double speed)
{
  if (vWindNED.Magnitude() == 0.0) {
    psiw = 0.0;
    vWindNED(eNorth) = speed*ktstofps;
  } else {
    vWindNED(eNorth) = speed * cos(psiw)*ktstofps;
    vWindNED(eEast) = speed * sin(psiw)*ktstofps;
    vWindNED(eDown) = 0.0;
  }
}
...
double FGWinds::GetWindspeedKts(void) const
{
  return vWindNED.Magnitude()*fpstokts;
}
...
// psi is the angle(deg) that the wind is blowing *towards*

void FGWinds::SetWindPsiDeg(double dir)
{
  double mag = GetWindspeed();
  psiw = dir*degtorad;
  SetWindspeed(mag);
...
void FGWinds::bind(void)
{
  ...
  //Modified for SMK
   PropertyManager->Tie("atmosphere/psiw-deg", this, &FGWinds::GetWindPsiDeg, &FGWinds::SetWindPsiDeg);
  ...
  //Modified for SMK
  PropertyManager->Tie("atmosphere/wind-mag-kts", this, &FGWinds::GetWindspeedKts,
                                                        &FGWinds::SetWindspeedKts);
