...
#include "DDSInterface/DDSInterface.h"
...
{
    //Added for Elevation model
  ElevationCalcFlag = PropertyManager->GetNode()->GetBool("ElevationModel/ElevationCalcFlag",0);
  CalculatedAGL = PropertyManager->GetNode()->GetDouble("ElevationModel/CalculatedAGL",0);
...
// Modified
    if (ElevationCalcFlag)
    {
        CrashDetectElevationModel();
    }
    else
    {
        ...
        double agl = PropertyManager->GetNode()->GetDouble("position/h-agl-ft",0);
        if (agl <= 10 && GearPos < 0.01) CrashDetectNotWOW();

        if (agl < -5) CrashDetectNegativeAltitude();
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
void FGLGear::CrashDetectNotWOW(void)
{
    if ( (vFn.Magnitude() > 500000.0 ||
        GetMoments().Magnitude() > 1500000.0 ||
        SinkRate > 1.4666*5 ) && !fdmex->IntegrationSuspended())
      {
        PutMessage("Crash Detected: Simulation FREEZE.");
        CrashStatus = true;
        initdds->QuitFromIOS = true;
        for (int i = 0; i<10; i++)
        {
            initdds->Run(false);
        }
...
void FGLGear::CrashDetectNegativeAltitude(void)
{
    CrashStatus = true;
    PutMessage("Crash Detected: Simulation FREEZE.");
    initdds->QuitFromIOS = true;
    cout<<"Quit neg: "<<initdds->QuitFromIOS<<endl;
    for (int i = 0; i<10; i++)
    {
        initdds->Run(false);
    }
...
void FGLGear::CrashDetectElevationModel()
{
    //cout<<"Calculated AGL: "<<CalculatedAGL<<endl;
    if (CalculatedAGL < -10)
    {
        CrashStatus = true;
        PutMessage("Crash Detected: Simulation FREEZE.");
